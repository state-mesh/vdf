...
func TestClassDiscriminant(t *testing.T) {
	...
	D := big.NewInt(-103)
	e_id := IdentityForDiscriminant(D)
	...
	e := NewClassGroupFromAbDiscriminant(big.NewInt(2), big.NewInt(1), D)
...
func TestSerialize1(t *testing.T) {
	x := NewClassGroup(big.NewInt(-0x10000), big.NewInt(-740), big.NewInt(4486780496))
	s := fmt.Sprintf("%02x", x.Serialize())
...
func TestDeSerialize1(t *testing.T) {
	...
	x, _ := NewClassGroupFromBytesDiscriminant(buf, big.NewInt(4486780496111111))
